...
const ProfileCardComponent: React.FC<ProfileCardProps> = ({
  avatarUrl = '/images/akash_jadhav.jpg',
  iconUrl,
  grainUrl,
  innerGradient,
  ...
  const cardStyle = useMemo(
    () => ({
      ...
      '--behind-glow-color': behindGlowColor ?? 'rgba(233, 230, 255, 0.4)',
  ...
  const shineStyle = {
    ...
    filter: 'brightness(0.5) contrast(1.2) opacity(0.3)',
    ...
    >
      {behindGlowEnabled && (
        <div
          ...
          style={{
            background: `radial-gradient(circle at var(--pointer-x) var(--pointer-y), rgba(255, 255, 255, 0.08) 0%, transparent var(--behind-glow-size))`,
            filter: 'blur(30px)',
            opacity: 0.6
          }}
        />
      ...
      <div ref={shellRef} className="relative z-[1] group">
        ...
        >
          ...
          >
            ...
            >
              <img
                className="w-full h-full object-cover absolute left-1/2 bottom-0 will-change-transform transition-transform duration-[120ms] ease-out opacity-90"
                src={avatarUrl}
            ...
            {/* Details content: Name & Role at Bottom */}
            <div
              className="max-h-full overflow-hidden text-center relative z-[10] flex flex-col justify-end p-6"
              style={{
                transform:
            ...
            >
              <div className="w-full flex flex-col items-center backdrop-blur-md bg-black/40 border border-white/10 p-4 rounded-2xl">
                <h3 className="font-archivo font-black text-[20px] tracking-tight text-white uppercase m-0 leading-tight">
                  {name}
                </h3>
                <p className="font-mono text-[11px] font-bold tracking-[0.08em] text-white/70 uppercase mt-1">
                  {title}
                </p>
              </div>
            </div>
